...
bool Turtlebot::command(double gx, double gy)
{
  ...
  if (counter2 == 0)
  {
    numAngles = (data_scan.angle_max - data_scan.angle_min) / data_scan.angle_increment;
    ROS_INFO("NUM ANGLES: %d\n", numAngles);
    for (int i = 0; i < numAngles; i++)
    {
      ROS_INFO("DATA SCAN AT %d: %f", i, data_scan.ranges.at(i));
  ...
  /**
	...
	*/
  numAngles = ceil((data_scan.angle_max - data_scan.angle_min) / data_scan.angle_increment);
  bool obstacle = false;
